...
% PURPOSE ----------
% Plot the results for a user specified number of domains from the most
% recent inversion run.  This script calls on the 3-D results array
% Master_Results_Table that sits in the workspace after the inversion. 

% INPUTS -----------


% The user needs to define the number of domains they want to plot and save
% the data for and the file name prefix (the file name will be completed by
% the script below --  see example).

ndomains = 6;  % the number of domains in the most recent model run to plot
filename_prefix = 'test6dom'  % just a name, no file extension!

% OUTPUTS ----------
% This script will produce two files, one pdf with an Arrhenius plot and a
% Delta plot (Lovera's r/ro), and a CSV with all of the observed and
...
%% get data user-specified model iteration results

tableSize = size(Master_Results_Table);
gotIT = 0;
i = 1;
while gotIT == 0
    iterDomains = Master_Results_Table(1,3,i)  % get the number of domains for this model iteration
    if ndomains == iterDomains
        domain_model_to_plot = Master_Results_Table(:,:,i);
        gotIT = 1;
        disp('Data Selected')
        
    elseif i > tableSize(3)
        gotIT = 1;
        disp('doh! I did not find your data!')
    end
    i = i+1;
end

% calculate
domain_model_to_plot = calc_domain_properties(obs_kinetics, domain_model_to_plot);
...
data_file = strcat(filename_prefix,'_modeled_data.csv');

% save files
csvwrite(data_file, domain_model_out_best)
